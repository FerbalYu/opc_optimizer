...
class TestSelfRepairPatches:
    ...
    def test_test_node_self_repair_rechecks_after_applying_patch(self, tmp_project, monkeypatch):
        ...
        with patch(
            ...
            return_value="[build] exit_code=1\nSyntaxError",
        ), patch(
    ...
    def test_test_failure_rolls_back_modified_files(self, tmp_project, monkeypatch):
        monkeypatch.setenv("OPC_MAX_SELF_REPAIR", "0")
        main_py = tmp_project / "main.py"
        main_py.write_text("def hello():\n    return 'broken'\n", encoding="utf-8")
        (tmp_project / "main.py.bak").write_text(
            "def hello():\n    return 'original'\n",
            encoding="utf-8",
        )
        ...
        with patch(
            ...
            "nodes.test._run_ui_check",
            return_value={"passed": True, "output": "UI verification disabled - skipped.", "skipped": True},
        ), patch("nodes.test._get_llm", return_value=FakeReviewLLM()):
            result = run_test_node(state)

        assert "return 'original'" in main_py.read_text(encoding="utf-8")
